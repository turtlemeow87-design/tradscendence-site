...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (import.meta.env.DEV || !import.meta.env.DATABASE_URL) {
    return json(200, { ok: true, message: 'Dev mode — registration skipped. Redirect to /verify.' });
  }
  ...
  try {
    ...
    await resend.emails.send({
      ...
      to: cleanEmail,
      subject: 'Your verification code — Tradscendence',
      html: `
        <div style="font-family: Georgia, serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #111; color: #F5F1E8; border: 1px solid #C2A45F;">
          <h2 style="color: #C2A45F; margin: 0 0 16px;">Verification Code</h2>
